...
describe("webhook adapters", () => {
  it("send Slack and Discord payloads to the local sink", async () => {
    ...
    expect(await discord.send(`${sink.url}/discord`, message)).toMatchObject({
      status: "delivered",
    });
    expect(sink.requests).toHaveLength(2);
    expect(sink.requests[0]?.body).toMatchObject({
      text: expect.stringContaining("Test alert"),
      blocks: expect.any(Array),
    });
    expect(sink.requests[1]?.body).toMatchObject({
      content: "It works",
  ...
  it("sends the expected request and verifies/parses callbacks", async () => {
    ...
    expect(
      adapter.verifyStatusWebhook(
        { "svix-id": "evt", "svix-timestamp": timestamp, "svix-signature": `v1 ${signature}` },
        raw,
      ),
    ).toBe(true);
    expect(
      adapter.verifyStatusWebhook(
        { "svix-id": "evt", "svix-timestamp": timestamp, "svix-signature": "v1 bad" },
        raw,
      ),
    ).toBe(false);
    expect(
      adapter.verifyStatusWebhook(
        { "svix-id": "evt", "svix-timestamp": "123", "svix-signature": `v1 ${signature}` },
        raw,
      ),
    ).toBe(false);
    expect(adapter.parseStatusEvent(JSON.parse(raw))).toEqual({
...
describe("TwilioSmsAdapter", () => {
  it("sends form data and verifies/parses callbacks", async () => {
    ...
    expect(await adapter.send("+15551111111", message)).toEqual({
      status: "sent",
    ...
    const webhookUrl = "https://app.example.test/webhooks/twilio";
    const params = new URLSearchParams(raw);
    const signedParams = [...params.keys()]
      .sort()
      .map((key) => `${key}${params.get(key) ?? ""}`)
      .join("");
    const signature = createHmac("sha1", "token")
      .update(`${webhookUrl}${signedParams}`)
      .digest("base64");
    expect(
      adapter.verifyStatusWebhook(
        { "x-twilio-signature": signature, "x-checkoutwatch-webhook-url": webhookUrl },
        raw,
      ),
    ).toBe(true);
    expect(
      adapter.verifyStatusWebhook(
        { "x-twilio-signature": "bad", "x-checkoutwatch-webhook-url": webhookUrl },
        raw,
      ),
    ).toBe(false);
    expect(adapter.parseStatusEvent(Object.fromEntries(new URLSearchParams(raw)))).toEqual({
      providerMessageId: "SM1",
      status: "delivered",
    });
  });
