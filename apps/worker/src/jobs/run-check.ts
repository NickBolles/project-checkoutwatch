...
export interface RunCheckPayload { monitorId: string; scheduledFor?: string; trigger?: "schedule" | "manual" | "recheck" }
export interface RunnableMonitor { id: string; productHandle: string; variantId?: string; storefrontUrl: string }
...
export function createRunCheckHandler(
  ...
  options: { timeoutMs?: number; staleLockMs?: number; now?: () => Date } = {},
) {
  const timeoutMs = options.timeoutMs ?? 15_000;
  const staleLockMs = options.staleLockMs ?? 5 * 60_000;
  const now = options.now ?? (() => new Date());
  return async (payload: RunCheckPayload, context: { id: string }): Promise<RunCheckDisposition> => {
    const jobKey = `job:${context.id}`;
    if (await repository.hasJobRun(jobKey)) return "duplicate";
    const acquiredAt = now();
    const monitor = await repository.acquireRunLock(payload.monitorId, acquiredAt, new Date(acquiredAt.getTime() - staleLockMs));
    if (!monitor) return "skipped";
    try {
      const result = await runner.run({
        storeUrl: monitor.storefrontUrl, productHandle: monitor.productHandle,
        ...(monitor.variantId ? { variantId: monitor.variantId } : {}), timeoutMs,
      });
