...
1. **Human (Nick):** create a Shopify **Partner account** + one **development store**; for each app, create the app in the Partner Dashboard and copy its **Client ID + Client secret** into that app's git-ignored `.env` (see §4). _Never paste secrets into chat or commit them._
...
> ⚠️ **The #1 risk theme (from the gap audits):** these apps are thoroughly correct against _mocks_. The untested surface is everything that only runs against _live_ Shopify / Redis / Postgres. AlertProof has a "production-shape adapter contract-test" suite as a template; expect the first real-store install to surface small shape mismatches. Budget time for that.
...
- **Image:** root `Dockerfile` (Node 20, `:3000`). _Builds clean (Dockerfile was fixed this session)._
...
- **Topology:** `web` + one standalone **worker** (Playwright) + **Postgres** + **Redis**. `docker-compose.yml` is the credential-free prod-parity stack (Postgres, Redis, fixture/control-probe, schema init, web, worker). `docker-compose.production.yml` is the VPS-ready stack: no fixture, private Postgres/Redis/worker, durable single-worker artifacts, and Traefik labels on web only; populate its git-ignored `.env.production` from `.env.production.example`.
...
- **Critical prod settings (audit-fixed, keep them right):**
  - **Stable shared `QUEUE_PREFIX`** across every web+worker replica (default `checkoutwatch`). _Never_ derive it from PID/container id — that was the fixed BLOCKER (web jobs never reached the worker).
