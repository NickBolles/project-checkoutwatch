  ...
  async tick(now = new Date()): Promise<number> {
    ...
    await Promise.all(
      due.map(async (monitor) => {
        ...
        if (!(await this.repository.claimSchedule(monitor.id, monitor.nextRunAt, nextRunAt)))
          return;
        await this.queue.add(
          "run-check",
          { monitorId: monitor.id, scheduledFor: monitor.nextRunAt.toISOString() },
          {
            jobId: `${monitor.id}:${monitor.nextRunAt.getTime()}`,
            attempts: 3,
